...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
 ...
 const getTweetIdsQuery = `select tweet_id

  from tweet where user_id=${getUserId.user_id};`;

 const getTweetIdsArray = await database.all(getTweetIdsQuery);

 const tweetIdsArray = getTweetIdsArray.map((eachId) => {

  return parseInt(eachId.tweet_id);

 });

 for (const tweetIdObj of tweetIdsArray) {

  const tweetId = tweetIdObj.tweet_id;

  const get_tweet_info = `select t.tweet as tweet,count(l.like_id) as likes ,count(r.reply_id) as replies, t.date_time 

  from tweet t inner join like l on t.tweet_id=l.tweet_id inner join reply r on r.tweet_id=l.tweet_id WHERE t.tweet_id = ? `;

  const tweetInfo = await database.get(get_tweet_info, [tweetId]);

  tweetsInfo.push(tweetInfo);

 }

 response.send(tweetsInfo.map((eachItem) => apiOutput_api9(eachItem)));
